...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
		 ...
		 if (Input.GetKey (KeyCode.LeftArrow)) { //walking left
			...
			if (Input.GetKeyUp (KeyCode.UpArrow)) { // if the up key is released while the player is walking, then stop jumping
		 ...
		 else if (Input.GetKey (KeyCode.RightArrow)) { //walking right
		...
		else 
		{
		 ...
		 if (Input.GetKey (KeyCode.DownArrow)) { //while up key is held down, player will continue to jump
		 ...
		 if (Input.GetKeyUp (KeyCode.DownArrow)) { //while up key is held down, player will continue to jump
		 ...
		 if (Input.GetKey (KeyCode.UpArrow)) { //while up key is held down, player will continue to jump
			
			animator.SetBool ("isJumping", true);
			transform.Translate(Vector3.up * jumpHeight * Time.deltaTime, Space.World);
		}
		if (Input.GetKeyUp (KeyCode.UpArrow)) { //When up key is released, player stops jumping
	...
	void OnTriggerStay2D (Collider2D other) {
		if (other.tag == "Shadow")
			other.SendMessageUpwards ("attack", swordDamage);
	}
}
